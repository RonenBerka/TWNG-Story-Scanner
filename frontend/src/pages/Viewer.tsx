...
import { fetchViewerData, downloadTWNGExport, type ViewerData } from "../lib/api";
      ...
      <div className="v-app">
        <aside className="v-sidebar">
          <div className="v-s-head">
            ...
            <button className="v-export-btn" onClick={handleExport} disabled={exporting || !data?.guitars.length}>
              {exporting ? "Exporting..." : "Export to TWNG"}
            </button>
            {exportMsg && <div className={`v-export-msg ${exportMsg.startsWith("Export failed") ? "err" : ""}`}>{exportMsg}</div>}
          </div>
...
.v-export-btn{display:block;width:100%;margin-top:8px;background:var(--v-amber);border:1px solid var(--v-amber-b);border-radius:6px;color:#080705;font-size:11px;font-family:'JetBrains Mono',monospace;font-weight:700;padding:7px 12px;cursor:pointer;transition:all .15s;letter-spacing:.04em}
.v-export-btn:hover{background:var(--v-amber-b);transform:translateY(-1px)}.v-export-btn:disabled{opacity:.4;cursor:not-allowed;transform:none}
.v-export-msg{margin-top:6px;font-size:10px;font-family:'JetBrains Mono',monospace;color:var(--v-green);padding:4px 8px;background:rgba(82,201,122,0.08);border-radius:4px}
.v-export-msg.err{color:var(--v-red);background:rgba(248,113,113,0.08)}
